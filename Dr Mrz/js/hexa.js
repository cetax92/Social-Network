    ...
    async function getPostUser() {
      ...
      if (comments.length > 0) {
        comments.forEach((comment) => {
          comments_html += `<div class="single-comment"><p style="font-weight:bold";>${user.username}:</p>${comment.content}</div>`;
        });
      }
      ...
      document.querySelector("#allPostsWrapper").innerHTML =
        `<div class="single-post" data-post_id="${post.id}">
                            <p><b>Posted by:</b> ${user.username}</p>
                            <div class="post-content">${post.content}</div>
                               ...
                               <div class="post-actions">
                                     ...
                                     <div>
                                       ...
                                       <button onclick="likePost(this)" class="likePostJS like-btn">Likes (<span>${post.likes}</span>)</button>
                                        <button class="comment-btn" onclick="commentPost(this)">Comments(<span>${comments.length}</span>)</button>
                                           ${delete_post_html}
                                      </div>
                                  </div>
...
const commentPost = (btn) => {
  ...
  if (main_post_el.querySelector(".post-commments").style.display === "block") {
    main_post_el.querySelector(".post-commments").style.display = "none";
  } else {
